...
try {
    ...
    if ($recent_otp) {
        $time_diff = strtotime('now') - strtotime($recent_otp['created_at']);
        $wait_time = 60 - $time_diff;
        
        http_response_code(429);
        echo json_encode([
            'status' => 'error', 
            'message' => "Please wait {$wait_time} seconds before requesting another OTP",
            'data' => [
    ...
    $stmt = $db->prepare("UPDATE password_reset_otps SET is_used = TRUE WHERE email = ? AND is_used = FALSE");
    $stmt->execute([$email]);
    ...
    if ($email_result['success']) {
        ...
        $response = [
            'status' => 'success',
            'message' => 'New OTP generated but email delivery failed. Please check your email or try again.',
            'data' => [
                'user_id' => $user['id'],
                'email' => $email,
                'expiry_minutes' => 10,
