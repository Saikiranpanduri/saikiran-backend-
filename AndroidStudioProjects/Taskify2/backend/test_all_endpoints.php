<?php
echo "=== Taskify Backend Complete Test ===\n\n";
...
// Test 4: Test getting tasks
echo "4. Testing get_tasks.php...\n";
$getTasksResponse = file_get_contents('http://localhost:8000/get_tasks.php');
if ($getTasksResponse) {
    $data = json_decode($getTasksResponse, true);
    if ($data && isset($data['success']) && $data['success']) {
        echo "   ✓ get_tasks.php working\n";
        echo "   Found " . count($data['data'] ?? []) . " tasks\n";
    } else {
        echo "   ✗ get_tasks.php returned error: " . ($data['message'] ?? 'Unknown') . "\n";
    }
} else {
    echo "   ✗ get_tasks.php failed\n";
}

echo "\n";

// Test 5: Test updating a task (if we created one)
if (isset($createdTaskId)) {
    echo "5. Testing update_task.php...\n";
    $updateData = [
        'id' => $createdTaskId,
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'http://localhost:8000/update_task.php');
    curl_setopt($ch, CURLOPT_POST, true);
    ...
    if ($error) {
        ...
        if ($response) {
            ...
            if ($decoded && isset($decoded['success']) && $decoded['success']) {
                ...
                echo "   ✗ Task update failed: " . ($decoded['message'] ?? 'Unknown error') . "\n";
            }
        } else {
            echo "   ✗ No response received\n";
        }
    ...
    // Test 6: Test deleting a task
    echo "6. Testing delete_task.php...\n";
    $deleteData = ['id' => $createdTaskId];
    
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, 'http://localhost:8000/delete_task.php');
    curl_setopt($ch, CURLOPT_POST, true);
    ...
    if ($error) {
        ...
        if ($response) {
            $decoded = json_decode($response, true);
            if ($decoded && isset($decoded['success']) && $decoded['success']) {
                echo "   ✓ Task moved to completed successfully!\n";
            } else {
                echo "   ✗ Task deletion failed: " . ($decoded['message'] ?? 'Unknown error') . "\n";
            }
        } else {
            echo "   ✗ No response received\n";
        }
    ...
    echo "5. Skipping update/delete tests (no task created)\n";
}
